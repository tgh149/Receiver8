...
import database
# --- MODIFIED: Use the new config system ---
...
def _get_session_path(phone_number: str, user_id: str, status: str, country_name: str):
    # --- MODIFIED: Use the SESSIONS_DIR from config ---
    config = get_config()
    folder_name = country_name.replace(" ", "_").lower()
    sessions_dir_path = os.path.join(config.SESSIONS_DIR, folder_name, status)
...
def _get_client_for_job(session_file: str, bot_data: dict):
    ...
    if api_credential:
        ...
        api_id, api_hash = int(bot_data.get('api_id', 25707049)), bot_data.get('api_hash', '676a65f1f7028e4d969c628c73fbfccc')
...
async def _perform_spambot_check(client: TelegramClient, spambot_username: str) -> tuple[str, str]:
    ...
    try:
        ...
        if not me: # Handle cases where client is not authorized
            return 'error', 'Client not authorized. Cannot perform spambot check.'
...
async def forward_session_to_log_channel(bot: Bot, bot_data: dict, account: dict, final_status: str, country_info: dict):
    ...
    while retries > 0:
        ...
        reg_time_dt = datetime.fromisoformat(account['reg_time']) if isinstance(account['reg_time'], str) else account['reg_time']
        
        caption = (f"*{category} Account*\n\n"
                   f"📱 Phone: `{escape_markdown(account['phone_number'])}`\n"
                   f"👤 User: {username} \\(`{account['user_id']}`\\)\n"
                   f"🗓️ Added: `{escape_markdown(reg_time_dt.strftime('%Y-%m-%d %H:%M'))}`")

        phone_no_plus = account['phone_number'].lstrip('+')
        json_content = json.dumps({"session_file": os.path.basename(session_file_path), "phone": account['phone_number'], "register_time": int(reg_time_dt.timestamp()), "status": final_status}, indent=4).encode('utf-8')
...
async def finalize_account_processing(bot: Bot, bot_data: dict, job_id: str, final_status: str, status_details: str, prompt_message_id: int | None = None):
    ...
    if not account or account['status'] not in ['pending_confirmation', 'pending_session_termination', 'error']:
        logger.warning(f"Finalize called for job {job_id} with invalid status: {account.get('status') if account else 'N/A'}")
        return
    ...
    if final_status in ['ok', 'restricted', 'limited', 'banned'] and country_info:
        # We need to refetch the account data to get the updated session_file path
        final_account_data = database.find_account_by_job_id(job_id)
        if final_account_data:
            await forward_session_to_log_channel(bot, bot_data, final_account_data, final_status, country_info)
        else:
            logger.warning(f"Could not refetch account data for job {job_id} during forwarding.")
    ...
    try:
        ...
        logger.error(f"Error sending finalization message to user {chat_id}: {e}")
    ...
    if not state:
        ...
        try:
            ...
            scheduler.add_job(
                ...
                args=[config.BOT_TOKEN, str(user_id), chat_id, state['phone'], job_id, success_msg.message_id], 
                id=job_id
            )
...
async def schedule_initial_check(bot_token: str, user_id_str: str, chat_id: int, phone_number: str, job_id: str, prompt_message_id: int | None):
    ...
    if not account or not account.get('session_file') or not os.path.exists(account.get('session_file')):
        ...
        # Only update status if the account exists
        if account:
            await finalize_account_processing(bot, bot_data, job_id, 'error', 'Session file lost during wait.', prompt_message_id)
        return
    ...
    try:
        ...
        if bot_data.get('enable_device_check') == 'True':
            ...
            if len(auths.authorizations) > 1:
                database.update_account_status(job_id, 'pending_session_termination', 'Multiple devices detected')
                ...
                # Don't finalize yet, let the cron job handle it.
                return
...
async def reprocess_account(bot: Bot, account: dict):
    ...
    try:
        ...
        spam_status, details = await _perform_spambot_check(client, bot_data.get('spambot_username'))
        # Reprocessing doesn't have a prompt_message_id, so we pass None
        await finalize_account_processing(bot, bot_data, job_id, spam_status, details, None)
    except Exception as e:
        logger.error(f"Job {job_id} (Reprocessing) failed: {e}", exc_info=True)
        await finalize_account_processing(bot, bot_data, job_id, 'error', f"Reprocessing failed: {e}", None)
    finally:
        if client and client.is_connected():
            await client.disconnect()
...
async def handle_account_status_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        scheduler.add_job(
            ...
            args=[config.BOT_TOKEN, str(account['user_id']), account['user_id'], account['phone_number'], job_id, prompt_message_id],
            id=job_id,
